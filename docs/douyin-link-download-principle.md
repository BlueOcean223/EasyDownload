...
2. 如果没选中，就回退到第一个可用流
3. 创建目标文件路径
4. 先用 `GET Range` 探测服务端是否真实支持 Range
5. 如果支持 Range 且文件足够大，则启用多分片下载
6. 每个分片必须返回 `206 Partial Content`，并校验 `Content-Range`
7. 否则回退到顺序下载
...
- 大文件尽量快
- 不支持 Range 的源站也能正常下载
- 避免只看 `HEAD Accept-Ranges` 导致服务器忽略 Range 时写坏文件
...
- App 重启后，抖音自定义下载闭包不会被持久化；未完成任务会标记为失败并提示重新解析后再下载
...
- 清晰度流 URL 失效
- 服务器不支持 Range，或 Range/`Content-Range` 校验失败
- 图集里的某一项媒体地址为空
- 临时目录或目标目录写入失败
